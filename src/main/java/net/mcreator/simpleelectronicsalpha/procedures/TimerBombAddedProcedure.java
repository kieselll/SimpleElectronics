package net.mcreator.simpleelectronicsalpha.procedures;

import net.minecraftforge.fml.common.Mod;
import net.minecraftforge.eventbus.api.SubscribeEvent;
import net.minecraftforge.eventbus.api.Event;
import net.minecraftforge.event.entity.player.PlayerEvent;

import net.minecraft.world.item.ItemStack;

import net.mcreator.simpleelectronicsalpha.init.SimpleElectronicsAlphaModBlocks;

import javax.annotation.Nullable;

@Mod.EventBusSubscriber
public class TimerBombAddedProcedure {
	@SubscribeEvent
	public static void onItemCrafted(PlayerEvent.ItemCraftedEvent event) {
		execute(event, event.getCrafting());
	}

	public static void execute(ItemStack itemstack) {
		execute(null, itemstack);
	}

	private static void execute(@Nullable Event event, ItemStack itemstack) {
		if (itemstack.getItem() == SimpleElectronicsAlphaModBlocks.TIMER_BOMB.get().asItem()) {
			itemstack.getOrCreateTag().putDouble("Timer1", 1000);
		}
	}
}
